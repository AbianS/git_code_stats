...
fn run_command(command: &str) -> String {
    ...
    str::from_utf8(&output.stdout).expect("Invalid UTF-8").to_string()
}
...
fn get_unique_authors() -> Vec<String> {
    ...
    let authors: Vec<String> = stdout.trim().split('\n').map(|author| author.trim().to_string()).collect();
    authors
}
...
fn get_git_stats(author: &str) -> (String, i32, i32) {
    ...
    let re: regex::Regex = regex::Regex::new(r"(\d+) files? changed, (\d+) insertions\(\+\), (\d+) deletions\(-\)").unwrap();
...
fn main() {
    ...
    let colors = vec![
        ...
        let handles: Vec<_> = authors
            ...
            .map(|(index, author)| {
                ...
                thread::spawn(move || {
                    let (author_name, lines_inserted, files_changed): (String, i32, i32) = get_git_stats(&author_clone);
    ...
    let data: Vec<Data> = author_stats
        .iter()
        .map(|(author, (lines_inserted, color))| {
            Data {
